...
def find_nearest_address_with_manual_distance(coords):
    """
    ...
    try:
        ...
        params = {
            "key": DGIS_API_KEY,
            "q": "дом, здание, строение",  # Более широкий запрос
            "point": f"{lon},{lat}",
            "fields": "items.point,items.full_name,items.address_name",
            "radius": 3000,  # Радиус поиска
            "sort": "distance",  # Сортировка по расстоянию
            "page_size": 5,  # Получить несколько ближайших объектов
        }
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()

        # Лог ответа API для отладки
        print("Response from Catalog API:", json.dumps(data, indent=4, ensure_ascii=False))

        if "result" in data and "items" in data["result"] and data["result"]["items"]:
            min_distance = float("inf")
            nearest_address = "No results found"
            for item in data["result"]["items"]:
                item_coords = (item["point"]["lat"], item["point"]["lon"])
                distance = geodesic(coords, item_coords).meters
                if distance < min_distance:
                    min_distance = distance
                    nearest_address = f"{item.get('address_name', 'Unknown address')} (Distance: {distance:.2f} m)"
...
def process_input():
    ...
    try:
        ...
        for i in range(len(usernames)):
            ...
            nearest_address = find_nearest_address_with_manual_distance(coords)
