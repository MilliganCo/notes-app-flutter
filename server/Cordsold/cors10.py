...
def find_nearest_address_with_debug(coords):
    """
    Найти ближайший адрес через Catalog API 2ГИС с ручной проверкой расстояния и отладкой.
    """
    try:
        lat, lon = coords
        ...
        params = {
            "key": DGIS_API_KEY,
            "q": "",  # Убрать фильтр для получения всех объектов
            "point": f"{lon},{lat}",
            "fields": "items.point,items.full_name,items.address_name",
            "radius": 1000,  # Уменьшенный радиус для точного поиска
            "sort": "distance",  # Сортировка по расстоянию
            "page_size": 10,  # Получить до 10 объектов
        }
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()

        # Отладка: вывод полного ответа
        print("Full response from Catalog API (address):", json.dumps(data, indent=4, ensure_ascii=False))
        ...
        if "result" in data and "items" in data["result"] and data["result"]["items"]:
            ...
            for item in data["result"]["items"]:
                ...
                # Отладка: вывод информации об объекте
                print(f"Object: {item.get('address_name', 'Unknown')} (Distance: {distance:.2f} m)")
...
def process_input():
    ...
    try:
        ...
        for i in range(len(usernames)):
            ...
            nearest_address = find_nearest_address_with_debug(coords)
