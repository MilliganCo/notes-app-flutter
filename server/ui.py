...
def create_ui():
        ...
        if lat is not None and lon is not None:
            ...
            if results:
                for note in results:
                    ...
                    # Показать каждую записку в отдельном окне
                    save = messagebox.askyesno("Сохранить записку?", f"{result_str}\n\nСохранить эту записку?")
                    if save and current_user:
    ...
    def generate_notes_ui():
        ...
        if count:
            notes = generate_random_notes(count)  # Генерация базовых записок
            enriched_notes = enrich_notes_with_geodata(notes)  # Обогащение адресом и метро
            insert_notes(enriched_notes)  # Вставка обогащённых записок в базу данных
