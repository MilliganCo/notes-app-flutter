...
def generate_random_coords(lat, lon, radius_meters):
    ...
    delta_lon = random.uniform(-radius_meters, radius_meters) / earth_radius * (180 / math.pi) / math.cos(lat * math.pi / 180)
    return lat + delta_lat, lon + delta_lon


# Функция для генерации случайных координат в пределах Москвы
def generate_random_coords_moscow():
...
def generate_notes():
    print("Выберите режим генерации записок:")
    print("1. Генерация в конкретном месте с определённым радиусом")
    print("2. Генерация в случайных местах Москвы")
    ...
    if choice == '1':
        ...
        def generate_coords():
            return generate_random_coords(lat_origin, lon_origin, radius)
    elif choice == '2':
        num_notes = int(input("Введите количество записок: "))

        def generate_coords():
            return generate_random_coords_moscow()
    else:
